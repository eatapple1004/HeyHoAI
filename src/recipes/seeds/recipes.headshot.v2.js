...
module.exports = [
  {
    ...
    "config": {
      "prompt_override": "업로드한 참조 인물 사진과 '동일 인물'을 최우선으로 엄격히 유지한 대한민국 표준 증명사진 1장(단일 인물, 단일 컷)을 만든다.\n\n[최우선 규칙: 동일 인물 고정(절대)] 참조 사진과 100% 동일 인물로만 생성한다. 얼굴이 달라지면 실패다. 얼굴 기하(landmarks)와 비율을 절대 바꾸지 않는다: 얼굴형, 이마/광대/턱선, 눈 사이 거리, 눈매/눈꼬리 각도, 눈 크기, 코 길이/폭/콧대, 입술 두께/입매, 귀 형태, 눈썹 형태, 인중 길이, 볼살, 주름/특징점, 안경 유무, 점/흉터/자국 등 고유 특징을 그대로 보존한다. 미화/성형/재해석/새 얼굴 생성을 절대 금지한다. 특히 눈 크게·턱 깎기·코 높이기·얼굴 축소·V라인·광대 축소·과도한 미백·피부를 플라스틱처럼 매끈하게·이목구비 위치 이동·얼굴 비율 재조정·젊게 만들기를 금지한다.\n\n[허용 편집: 증명사진 수준의 최소 보정만] 헤어는 참조의 구조(가르마/길이/앞머리 유무/결 방향)를 100% 유지한 채 잔머리·부스스함만 자연스럽게 정리한다(앞머리 스타일 변경·직모↔웨이브 전환·길이 큰 변경·묶음/풀림 전환 금지). 피부는 잡티·트러블·번들거림·다크서클·홍조를 아주 약하게만 완화하되 모공·잔주름·결 등 질감을 유지한다(과한 블러/필터/미백/윤곽 보정 금지).\n\n[대한민국 증명사진 스타일] 정면, 시선 정면, 고개 기울임 최소, 어깨 수평, 머리·어깨 중심 정렬. 무표정 또는 아주 약한 미소(치아 보이지 않게). 균일한 스튜디오 조명, 그림자 최소, 디테일 선명. 배경은 연한 하늘색 단색(완전 평면 — 그라데이션/패턴/소품/텍스트/그림자 금지).\n\n[의상] 깔끔한 정장(검정/네이비/차콜 재킷 + 흰색 또는 연한 톤 셔츠/블라우스, 무늬·큰 로고 금지). 남성은 단색 넥타이(검정/네이비/차콜, 무늬 없음) 필수, 여성은 넥타이 없이 단정한 셔츠/블라우스. 의상보다 얼굴 동일성 유지가 우선이다.\n\n[크롭/규격] 2x2인치(약 5x5cm) 정방형 비율, 머리 위 여백 5~10%, 어깨선~가슴 윗부분까지 보이는 전형적 증명사진 프레이밍.\n\n[출력] 한 명·한 컷의 단일 증명사진 1장만 출력한다. 여러 컷을 한 이미지에 배열 금지(콜라주/그리드/2x2 시트/연속컷 금지), 글자/워터마크/로고/날짜/QR/테두리 등 모든 부가요소 금지. 고해상도, 인쇄해도 선명한 품질.",
      "schema_version": 1,
      "mode": "influencer",
      "output": {
        "type": "image_set",
