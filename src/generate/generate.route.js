...
router.get('/creations/:idx', async (req, res, next) => {
  try {
    ...
    if (r.minted_template_id) {
      const m = await query(`SELECT mt.id, mt.name, mt.price_credits, COALESCE(mt.preview_media->>0, (SELECT '/'||regexp_replace(gr.file_path,'^tmp/','') FROM generation_results gr WHERE gr.template_id = mt.id::text AND gr.visibility='public' AND gr.file_path IS NOT NULL AND gr.idx <> $3 ORDER BY gr.likes_count DESC NULLS LAST, gr.idx DESC LIMIT 1)) AS thumb, (mt.creator_id = $2) AS mine FROM marketplace_templates mt WHERE mt.id = $1 AND mt.status = 'active'`, [r.minted_template_id, req.user.id, idx]);
      ...
      const m = await query(`SELECT m.id, m.name, m.price_credits, COALESCE(m.preview_media->>0, (SELECT '/'||regexp_replace(gr.file_path,'^tmp/','') FROM generation_results gr WHERE gr.template_id = m.id::text AND gr.template_source='marketplace' AND gr.visibility='public' AND gr.file_path IS NOT NULL AND gr.idx <> $3 ORDER BY gr.likes_count DESC NULLS LAST, gr.idx DESC LIMIT 1)) AS thumb, (m.creator_id = $2) AS mine, (m.is_official OR EXISTS(SELECT 1 FROM template_owns o WHERE o.template_id = m.id AND o.user_id = $2)) AS owned FROM marketplace_templates m WHERE m.id::text = $1 AND m.status = 'active'`, [r.template_id, req.user.id, idx]);
      ...
      const m = await query(`SELECT mt.id, mt.name, mt.price_credits, COALESCE(mt.preview_media->>0, (SELECT '/'||regexp_replace(gr.file_path,'^tmp/','') FROM generation_results gr WHERE gr.template_id = mt.recipe_id AND gr.template_source='recipe' AND gr.visibility='public' AND gr.file_path IS NOT NULL AND gr.idx <> $2 ORDER BY gr.likes_count DESC NULLS LAST, gr.idx DESC LIMIT 1)) AS thumb FROM marketplace_templates mt WHERE mt.recipe_id = $1 AND mt.is_official = true AND mt.status = 'active' LIMIT 1`, [r.template_id, idx]);
