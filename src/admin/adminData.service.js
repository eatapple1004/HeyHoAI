...
// 관리자 데이터 조회 로직 단일소스 — 레거시 라우트(adminData.route.js)와 Nest(nest/admin) 공용.
//   읽기 전용. 각 블록은 개별 try/catch로 격리 → 테이블·컬럼 누락 환경에서도 부분 렌더(전체 500 방지).
...
/**
 * 전체 크리에이션(비공개 포함) 목록 + 필터
 * @param {{visibility?:string, status?:string, q?:string, limit?:string, offset?:string}} q
 * @returns {{data:object[], hasMore:boolean}}
 */
async function listCreations(q = {}) {
  const limit = Math.min(Math.max(parseInt(q.limit, 10) || 60, 1), 200);
  const offset = Math.max(parseInt(q.offset, 10) || 0, 0);
  const where = [];
  const params = [];
  if (q.visibility === 'public' || q.visibility === 'private') {
    params.push(q.visibility); where.push(`gr.visibility = $${params.length}`);
  }
  if (['success', 'failed', 'pending'].includes(q.status)) {
    if (q.status === 'failed') where.push(`gr.status <> 'success'`);
    else { params.push(q.status); where.push(`gr.status = $${params.length}`); }
  }
...
/**
 * 대시보드 통계 — 유저 / 생성물(이미지·영상 구분) / 기능별 사용량(영상·광고UGC·온모델·팩) /
 * 매출 / 템플릿·모델 / 헤비유저 / 인게이지먼트 / 14일 시계열.
 */
...
module.exports = { toUrl, isReel, listCreations, getStats };
