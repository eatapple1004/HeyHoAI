 ...
 *
 * ⚠️ **메뉴를 고쳤으면 모든 어드민 페이지의 `?v=` 를 같이 올릴 것.**
 *   정적 자산이 max-age 4시간이라, 여기만 고치면 브라우저가 옛 파일을 계속 쓴다 —
 *   실제로 '결제' 탭을 추가했는데 새 페이지(v=2)에서만 보이고 나머지(v=1)엔 안 나왔다.
 *   `sed -i '' 's|admin-nav\.js?v=N|admin-nav.js?v=N+1|g' public/admin-*.html`
 */
(function () {
  var PAGES = [
