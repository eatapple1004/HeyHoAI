...
export class TemplateDataController {
  ...
  @Post()
  async create(@Req() req: any, @Body() body: CreateTemplateDataDto): Promise<ApiResponse<TemplateDataVo>> {
    return { success: true, data: await this.templateData.create(req.user.id, body) };
  }

  // GET /api/template-data?templateType=&characterId=
  @Get()
  async list(@Req() req: any, @Query() q: ListTemplateDataQueryDto): Promise<ApiResponse<TemplateDataVo[]>> {
    return { success: true, data: await this.templateData.list(req.user.id, q) };
  }

  // GET /api/template-data/:id
  @Get(':id')
  async getById(@Req() req: any, @Param('id') id: string): Promise<ApiResponse<TemplateDataVo>> {
    return { success: true, data: await this.templateData.getById(req.user.id, id) };
  }

  // PATCH /api/template-data/:id
  @Patch(':id')
  async update(@Req() req: any, @Param('id') id: string, @Body() body: UpdateTemplateDataDto): Promise<ApiResponse<TemplateDataVo>> {
    return { success: true, data: await this.templateData.update(req.user.id, id, body) };
  }

  // DELETE /api/template-data/:id
  @Delete(':id')
  async remove(@Req() req: any, @Param('id') id: string): Promise<ApiResponse<TemplateDataVo>> {
