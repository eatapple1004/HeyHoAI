...
import { ThemeVo, TemplateCardVo, OwnedTemplateVo } from './vo/template.vo';
import {
  CreatorMeDto, EarningsDto, ApplyCreatorResultDto, CreatorStorefrontDto, FollowResultDto,
  RecipeGateDto, DefaultOfficialDto, OwnedInStudioResultDto, SetOwnedInStudioDto,
...
export class MarketplaceCreatorsController {
  ...
  async apply(@Req() req: any): Promise<ApiResponse<ApplyCreatorResultDto>> {
    return { success: true, data: await this.marketplace.applyCreator(req.user.id) };
  }

  // GET /api/marketplace/creators/:handle — 공개 스토어프론트
  @Get('creators/:handle')
  async creator(@Req() req: any, @Param('handle') handle: string): Promise<ApiResponse<CreatorStorefrontDto>> {
  ...
  @HttpCode(200)
  async follow(@Req() req: any, @Param('handle') handle: string): Promise<ApiResponse<FollowResultDto>> {
  ...
  async unfollow(@Req() req: any, @Param('handle') handle: string): Promise<ApiResponse<FollowResultDto>> {
  ...
  @Get('bookmarks')
  async bookmarks(@Req() req: any): Promise<ApiResponse<TemplateCardVo[]>> {
  ...
  @Get('recipe-gates')
  async recipeGates(@Req() req: any): Promise<ApiResponse<RecipeGateDto[]>> {
  ...
  @Get('owned')
  async owned(@Req() req: any): Promise<ApiResponse<OwnedTemplateVo[]>> {
  ...
  async ownedInStudio(@Req() req: any, @Body() body: SetOwnedInStudioDto): Promise<ApiResponse<OwnedInStudioResultDto>> {
  ...
  async defaultOfficials(@Req() req: any): Promise<ApiResponse<DefaultOfficialDto[]>> {
