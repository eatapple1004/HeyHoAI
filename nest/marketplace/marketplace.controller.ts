...
import { TemplateCardVo } from './vo/template.vo';
import {
  ...
  ApiWithCharged, ListTemplatesQueryDto, CreateTemplateDto, UpdateTemplateDto, ReportTemplateDto,
...
export class MarketplaceController {
  ...
  @Get()
  async list(@Req() req: any, @Query() q: ListTemplatesQueryDto): Promise<ApiResponse<TemplateCardVo[]>> {
  ...
  @Post()
  async create(@Req() req: any, @Body() body: CreateTemplateDto): Promise<ApiResponse<SavedTemplateDto>> {
  ...
  @Get(':id')
  async detail(@Req() req: any, @Param('id') id: string): Promise<ApiResponse<TemplateDetailDto>> {
    return { success: true, data: await this.marketplace.getTemplate(req.user.id, id) };
  }

  // GET /api/marketplace/templates/:id/creations
  @Get(':id/creations')
  async creations(@Param('id') id: string): Promise<ApiResponse<TemplateCreationDto[]>> {
    return { success: true, data: await this.marketplace.getTemplateCreations(id) };
  }

  // PATCH /api/marketplace/templates/:id — 내 템플릿 편집
  @Patch(':id')
  async update(@Req() req: any, @Param('id') id: string, @Body() body: UpdateTemplateDto): Promise<ApiResponse<SavedTemplateDto>> {
  ...
  @Delete(':id')
  async remove(@Req() req: any, @Param('id') id: string): Promise<ApiResponse<DeletedTemplateDto>> {
  ...
  async use(@Req() req: any, @Param('id') id: string): Promise<ApiWithCharged<UseTemplateDto>> {
    try {
  ...
  @Post(':id/acquire')
  @HttpCode(200)
  async acquire(@Req() req: any, @Param('id') id: string): Promise<ApiWithCharged<AcquireResultDto>> {
    try {
  ...
  @HttpCode(200)
  async addToMine(@Req() req: any, @Param('id') id: string): Promise<ApiResponse<AddToMyTemplatesResultDto>> {
  ...
  @Post(':id/report')
  @HttpCode(200)
  async report(@Req() req: any, @Param('id') id: string, @Body() body: ReportTemplateDto): Promise<ApiResponse<ReportResultDto>> {
  ...
  @Post(':id/bookmark')
  @HttpCode(200)
  async bookmark(@Req() req: any, @Param('id') id: string): Promise<ApiResponse<BookmarkResultDto>> {
  ...
  @Delete(':id/bookmark')
  async unbookmark(@Req() req: any, @Param('id') id: string): Promise<ApiResponse<BookmarkResultDto>> {
