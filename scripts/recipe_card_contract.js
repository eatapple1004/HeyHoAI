...
const VERTICALS = ['influencer', 'fashion', 'beauty', 'jewelry', 'food', 'coffee', 'home', 'tech', 'pet', 'ugc', 'general', 'headshot', 'productcut', 'studiomodel', 'producthero', 'accessories', 'bodywear'];
...
for (const v of VERTICALS) {
  ...
  for (const r of v2) {
    ...
    if (!pid) continue;
    (childrenByParent[pid] = childrenByParent[pid] || []).push({ id: slug(r.name), name: r.name, preview: r.preview || null, ...(r.meta && r.meta.fit ? { fit: r.meta.fit } : {}), ...(r.meta && r.meta.garment ? { garment: r.meta.garment } : {}) });
  }
